...
const ParentView: React.FC<ViewProps> = ({ isAiOpen, setIsAiOpen }) => {
  const [language, setLanguage] = useState<'en' | 'es'>('en');
  const [chartMode, setChartMode] = useState<'yearly' | 'monthly'>('yearly');
  const [isReportCardOpen, setIsReportCardOpen] = useState(false);
  
  // Hint State for Risk Score Card
  const [showRiskHint, setShowRiskHint] = useState(false);
  const [hasClickedRisk, setHasClickedRisk] = useState(false);
  
  const [aiPrompt, setAiPrompt] = useState<string | undefined>(undefined);

  const student = MOCK_STUDENT;

  // Trigger Risk Hint after delay
  useEffect(() => {
    if (hasClickedRisk) return;
    const timer = setTimeout(() => setShowRiskHint(true), 1500);
  ...
  const handleRiskClick = () => {
      setHasClickedRisk(true); // Disable hint
      setShowRiskHint(false);
      const prompt = language === 'es' 
        ? "¿Puede explicar qué significa el 'Puntaje de Riesgo' y por qué está en " + student.riskScore + "?"
        : "Can you explain what the 'Risk Score' means and why it is at " + student.riskScore + "?";
      launchAi(prompt);
  };
  ...
  return (
    <div className="relative min-h-screen bg-gray-50">
      ...
      {/* EdAssist AI Sidebar */}
      <EdAssistAI isOpen={isAiOpen} onClose={() => setIsAiOpen(false)} contextData={parentContext} initialPrompt={aiPrompt} role="Parent" language={language} />
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[70] flex items-center justify-center p-4 animate-fadeIn">
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full overflow-hidden flex flex-col max-h-[90vh]">
                <div className="bg-vt-blue p-4 flex justify-between items-center text-white">
                    ...
                    <button onClick={() => setIsReportCardOpen(false)} className="hover:bg-vt-darkBlue p-1 rounded transition">
                        <X size={20} />
                    </button>
                </div>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-6">
                        <div className="flex justify-between items-start border-b border-gray-100 pb-4 mb-4">
                            ...
                            <div className="text-right">
                                ...
                                <button className="text-vt-blue text-sm flex items-center gap-1 justify-end mt-1 hover:underline">
      ...
      <div className={`transition-all duration-300 ease-in-out ${isAiOpen ? 'mr-0 md:mr-[450px]' : ''}`}>
        <div className="p-6 max-w-7xl mx-auto space-y-8 pb-24">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">{t.welcome} <span className="text-vt-blue">{student.name}</span></h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div onClick={handleRiskClick} className="relative bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-4 cursor-pointer hover:shadow-md hover:border-vt-lightBlue transition">
                    <div className={`p-3 rounded-full ${student.riskScore > 50 ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-vt-blue'}`}>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
                    ...
                    <button onClick={() => launchAi(language === 'es' ? "Tengo una conferencia de padres y maestros el 15 de noviembre. Basado en los datos de mi hijo, ¿cuáles son 3 preguntas clave que debería hacerle al maestro?" : "I have a parent-teacher conference on Nov 15. Based on my child's data, what are 3 key questions I should ask the teacher?")} className="text-vt-blue hover:bg-blue-50 p-2 rounded-full transition cursor-pointer hover:shadow-md">
                        <Mail size={20} />
                    </button>
                </div>
            </div>
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-1 text-xs font-bold">
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded transition ${chartMode === 'yearly' ? 'bg-white text-vt-blue shadow-sm' : 'text-gray-500'}`}
                        >
                            {t.chart.yearly}
                        </button>
                        <button 
                            onClick={() => setChartMode('monthly')}
                            className={`px-3 py-1.5 rounded transition ${chartMode === 'monthly' ? 'bg-white text-vt-blue shadow-sm' : 'text-gray-500'}`}
                        >
                            {t.chart.monthly}
                        </button>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            ...
                            <Line 
                                ...
                                dataKey="score" 
                                name="Risk Score" 
                                stroke={chartMode === 'yearly' ? '#BF5700' : '#f59e0b'} 
                                strokeWidth={3} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col">
                    <h3 className="font-bold text-lg text-gray-800 mb-6 flex items-center gap-2">
                        <TrendingUp size={20} className="text-vt-blue"/> {t.goals}
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                        {student.goals.map(goal => (
                            <div key={goal.id} className="bg-gray-50 rounded-xl p-5 border border-gray-100 hover:border-vt-lightBlue transition">
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wide ${goal.type === 'IEP/504' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-vt-blue'}`}>
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div className={`h-2 rounded-full ${goal.type === 'IEP/504' ? 'bg-purple-500' : 'bg-vt-blue'}`} style={{width: `${goal.progress}%`}}></div>
                                </div>
                            </div>
                        ))}
                    </div>
