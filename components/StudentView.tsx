...
const StudentView: React.FC = () => {
      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8 pb-24 bg-sky-50 min-h-screen font-sans">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
             ...
             <span className="font-bold text-blue-800 text-lg bg-blue-100 px-4 py-1 rounded-full">Pecan Springs Elementary</span>
          </div>
          <button
        ...
        {/* Hero Section */}
        <div className="bg-gradient-to-r from-orange-400 to-amber-500 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="text-center md:text-left">
                    ...
                    <p className="text-amber-100 text-xl font-medium">Ready for a great day of learning?</p>
                </div>
  ...
  // ----------------------------------------------------------------------
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-10 pb-32 min-h-screen bg-gradient-to-br from-slate-50 via-gray-100 to-orange-50/20">
      ...
      {isAddArtifactOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden transform transition-all scale-100 border border-white/20">
                <div className="bg-vt-blue p-5 flex justify-between items-center text-white">
                ...
                <div className="p-6 space-y-5 bg-gray-50/50">
                    ...
                    <div className="pt-2">
                        <button 
                            onClick={handleSaveArtifact}
                            disabled={!newArtifact.title || isUploading}
                            className={`w-full py-3 rounded-xl font-bold shadow-lg transition flex items-center justify-center gap-2 ${!newArtifact.title || isUploading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-vt-blue text-white hover:bg-vt-lightBlue hover:shadow-xl active:scale-[0.98]'}`}
                        >
                            {isUploading ? (
                                <>
      ...
      {isAddGoalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all scale-100 border border-white/20">
                <div className="bg-gradient-to-r from-vt-blue to-orange-700 p-5 flex justify-between items-center text-white">
                ...
                <div className="p-6 space-y-5 bg-gray-50/50">
                    ...
                    <button 
                        ...
                        className="w-full bg-vt-blue text-white py-3 rounded-xl font-bold hover:bg-vt-lightBlue shadow-lg hover:shadow-xl transition transform active:scale-[0.98]"
                    >
                        Create Goal
                    </button>
                </div>
      ...
      <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 bg-vt-blue rounded-full flex items-center justify-center shadow-sm text-xl">
                🦁
             </div>
             <span className="font-bold text-vt-darkBlue text-lg bg-white px-4 py-1.5 rounded-full shadow-sm border border-blue-100">Capital City High School</span>
          </div>
          <button
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-3xl p-6 md:p-8 shadow-xl border border-white/50 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-orange-400/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
        
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative z-10">
            <div>
                <div className="text-sm font-bold text-vt-blue uppercase tracking-wider mb-1">Student Dashboard</div>
      ...
      <section>
        <div className="flex items-center justify-between mb-6 pl-1">
            <div className="flex items-center space-x-3">
                <div className="p-2 bg-blue-100 rounded-lg text-vt-blue shadow-sm"><Target size={24} /></div>
                <h2 className="font-bold text-2xl text-gray-800">Current Goals</h2>
            </div>
            <button 
                onClick={() => setIsAddGoalOpen(true)}
                className="group flex items-center gap-2 px-4 py-2 rounded-xl border border-blue-200 text-vt-blue font-bold text-sm hover:bg-blue-50 hover:border-blue-300 transition bg-white shadow-sm"
            >
        ...
        <div className="flex gap-6 overflow-x-auto pb-6 snap-x scrollbar-hide">
          {student.goals.map((goal) => (
            <div key={goal.id} className="min-w-[320px] md:min-w-[380px] bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-white/60 hover:shadow-xl hover:border-blue-200 transition-all duration-300 snap-center group relative overflow-hidden">
              {/* Decoration */}
              <div className={`absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl opacity-20 -translate-y-1/2 translate-x-1/2 pointer-events-none ${goal.type === 'IEP/504' ? 'bg-purple-500' : 'bg-vt-blue'}`}></div>
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                  <span className={`text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wide shadow-sm ${
                      ...
                      goal.type === 'Academic' ? 'bg-blue-100 text-vt-blue' : 'bg-gray-100 text-gray-600'
                  }`}>
                      {goal.type}
                  </span>
              ...
              <h3 className="font-bold text-xl text-gray-800 mb-2 leading-tight relative z-10 group-hover:text-vt-blue transition-colors">{goal.title}</h3>
              ...
              <div className="relative z-10">
                ...
                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden shadow-inner">
                    <div
                    className={`h-full rounded-full relative ${goal.type === 'IEP/504' ? 'bg-gradient-to-r from-purple-500 to-fuchsia-500' : 'bg-gradient-to-r from-vt-blue to-orange-500'}`}
                    style={{ width: `${goal.progress}%` }}
                    >
          ...
          <button 
            onClick={() => setIsAddGoalOpen(true)}
            className="min-w-[100px] bg-white/40 border-2 border-dashed border-gray-300 rounded-3xl flex flex-col items-center justify-center text-gray-400 hover:bg-white/80 hover:border-blue-300 hover:text-vt-blue transition cursor-pointer snap-center"
          >
