...
import { Sparkles, Search, Filter, AlertCircle, BrainCircuit, TrendingUp, X, BookOpen, Share, Save, Send, Mail, Phone, ArrowRight, CheckCircle, GraduationCap } from 'lucide-react';
...
const TeacherView: React.FC<ViewProps> = ({ isAiOpen, setIsAiOpen }) => {
    ...
    const [showTrendHint, setShowTrendHint] = useState(false);
    const [hasClickedTrend, setHasClickedTrend] = useState(false);
    ...
    // Trigger Hints
    useEffect(() => {
        if (hasClickedGroup) return;
        const timer = setTimeout(() => setShowGroupHint(true), 1500);
        return () => clearTimeout(timer);
    }, [hasClickedGroup]);

    useEffect(() => {
        if (hasClickedTrend) return;
        const timer = setTimeout(() => setShowTrendHint(true), 2500); // Stagger slightly after grouping hint
        return () => clearTimeout(timer);
    }, [hasClickedTrend]);
    ...
    const handleGroupClick = () => {
        setHasClickedGroup(true);
        setShowGroupHint(false);
        launchAi("Group these students for math intervention based on risk scores.");
    };

    const handleTrendClick = () => {
        setHasClickedTrend(true);
        setShowTrendHint(false);
        launchAi("Analyze the correlation between attendance and math performance for this class.");
    };
    ...
    return (
        <div className="relative overflow-x-hidden min-h-screen bg-gray-50">
                        ...
                        <div className="flex border-b border-gray-200 bg-gray-50 px-6">
                            {['overview', 'trends', 'conference'].map((tab) => (
                                <button
                                    ...
                                    className={`px-6 py-4 font-medium text-sm border-b-2 transition-colors capitalize ${activeModalTab === tab ? 'border-vt-blue text-vt-blue' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                                >
                                ...
                                <div className="space-y-6 animate-fadeIn">
                                    <div className="grid grid-cols-3 gap-4">
                                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-center">
                                            <div className="text-xs text-vt-blue font-bold uppercase tracking-wider mb-1">Attendance</div>
                                    ...
                                    <div>
                                        <h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2">
                                            <AlertCircle size={18} className="text-vt-blue" /> Active Goals
                                        </h3>
                                        <div className="space-y-3">
                                            {selectedStudent.goals.map((g, i) => (
                                                <div key={i} className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm flex justify-between items-center">
                                                    ...
                                                    <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                        <div className="h-full bg-vt-blue rounded-full" style={{ width: `${g.progress}%` }}></div>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                            ...
                            {activeModalTab === 'trends' && (
                                <div className="space-y-6 animate-fadeIn">
                                    <div className="flex items-start gap-4 bg-blue-50 p-4 rounded-xl border border-blue-100">
                                        <div className="bg-white p-2 rounded-lg shadow-sm text-vt-blue"><BrainCircuit size={24} /></div>
                                        <div>
                                            <h4 className="font-bold text-blue-900 text-sm">Explainable AI Insight</h4>
                                            <p className="text-sm text-blue-800 mt-1 leading-relaxed">
                                                EDU AI analysis identifies a correlation: The <strong>dip in 4th Grade Math scores</strong> correlates with a <strong>15% drop in attendance</strong> during that same academic year.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm h-64">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={longitudinalData}>
                                                ...
                                                <Tooltip />
                                                <Legend />
                                                <Line type="monotone" dataKey="math" name="Math RIT" stroke="#BF5700" strokeWidth={3} />
                            ...
                            {activeModalTab === 'conference' && (
                                <div className="space-y-8 animate-fadeIn">
                                    <div className="bg-indigo-50 rounded-xl p-6 border border-indigo-100">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-indigo-700 mt-1">Auto-generated by EDU AI based on longitudinal data.</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="border-t border-gray-100 pt-6">
                                        <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2"><Share size={18} className="text-vt-blue" /> Post-Conference Action</h3>
                                        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
                                            ...
                                            <div className="flex justify-end pt-2">
                                                <button onClick={handleCreatePLP} className="bg-vt-blue hover:bg-vt-lightBlue text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md flex items-center gap-2">
            ...
            {/* EduAI Sidebar */}
            <EduAI isOpen={isAiOpen} onClose={() => setIsAiOpen(false)} contextData={rosterContext} initialPrompt={aiPrompt} role="Teacher" />
                ...
                <div className="p-6 max-w-7xl mx-auto pb-24">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-vt-blue rounded-full flex items-center justify-center shadow-sm text-xl text-white">
                                🦁
                            </div>
                            <span className="font-bold text-blue-900 text-lg bg-white px-4 py-1.5 rounded-full shadow-sm border border-blue-100">Capital City High School</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => launchAi()} className="px-4 py-2 bg-gradient-to-r from-vt-blue to-vt-lightBlue text-white rounded-lg font-medium shadow-md flex items-center gap-2 hover:shadow-lg transition hover:brightness-110">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div onClick={handleGroupClick} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition cursor-pointer group relative">
                            <div className="w-10 h-10 bg-blue-100 text-vt-blue rounded-lg flex items-center justify-center mb-3"><BrainCircuit size={20} /></div>
                            ...
                            {/* Purple Guided Highlight - Grouping */}
                            {showGroupHint && (
                                <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 z-20 animate-bounce pointer-events-none min-w-[150px]">
                        ...
                        {/* Analyze Trends Tile */}
                        <div onClick={handleTrendClick} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition cursor-pointer group relative">
                            ...
                            {/* Purple Guided Highlight - Trends */}
                            {showTrendHint && (
                                <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 z-20 animate-bounce pointer-events-none min-w-[150px]">
                                    <div className="bg-purple-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-xl border-2 border-white flex items-center justify-center gap-2">
                                        <Sparkles size={12} className="text-yellow-300 animate-pulse" />
